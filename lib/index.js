...
class CodePointsIterator {
    ...
    previous() {
        this.output.done = false;
        if (--this.i >= 0) {
            const bytes = [this[BYTES][this.i]];
            if (bytes[0] & 0x80) {
                if ((this[BYTES][--this.i] & 0x40) === 0) {
                    bytes.unshift(this[BYTES][this.i]);
                    if ((this[BYTES][--this.i] & 0x40) === 0) {
                        bytes.unshift(this[BYTES][this.i]);
                        if ((this[BYTES][--this.i] & 0x40) === 0) {
                            bytes.unshift(this[BYTES][this.i]);
                ...
                bytes.unshift(this[BYTES][this.i]);
                // TODO this is suboptimal, so should be looked into later
                const tempBytes = this[BYTES];
                const tempI = this.i;
                this.i = -1;
                this[BYTES] = bytes;
                this.next();
